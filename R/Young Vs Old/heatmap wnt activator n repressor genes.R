...
markers <- read.csv("wnt activator or repressor.csv", header = TRUE)
...
pdf("boxplot_wnt_inhibitor_genes.pdf")
for(i in rownames(named_nc_markers_inhibition)){
  ...
  boxplot <- ggplot(boxcounts, aes(x=V2, y=V1)) +
    geom_boxplot(color = c("light grey", "light green")) +
    geom_point() +
    ggtitle(i) +
    xlab("age group") +
    ylab("FPKM")
...
pdf("boxplot_wnt_activator_genes.pdf")
...
pdf("barchart wnt inhibitor genes")
par(mfrow=c(2,2))
for(i in rownames(named_nc_markers_inhibition)){
...
pdf("barchart wnt activator genes")
par(mfrow=c(2,2))
for(i in rownames(named_nc_markers_activation)){
