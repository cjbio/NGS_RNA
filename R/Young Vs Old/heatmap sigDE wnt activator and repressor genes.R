...
#markers <- read.csv("~/database/wnt_chip_primers.csv", header = TRUE)
#markers <- read.csv("~/database/wnt target genes human ensemble ID final.csv", header = TRUE)
markers <- read.csv("~/database/wnt activator or repressor.csv", header = TRUE)
...
nc_markers <- as.data.frame(sig_filtered_nc[rownames(sig_filtered_nc) %in% rownames(markers),])
...
named_nc_markers_activation<- named_nc_markers[named_nc_markers$inhibition=="activation",]
named_nc_markers_inhibition<- named_nc_markers[named_nc_markers$inhibition=="inhibition",]


named_nc_markers_activation<- named_nc_markers_activation[,-c(1:6)]
named_nc_markers_inhibition<- named_nc_markers_inhibition[,-c(1:6)]


##################boxplot
coldata$condition <- relevel(coldata$condition, ref = "young")

pdf("boxplot_sigDE_wnt_inhibitor_genes.pdf")
for(i in rownames(named_nc_markers_inhibition)){
  boxcounts <- as.data.frame(t(named_nc_markers_inhibition))
...
pdf("boxplot_sigDE_wnt_activator_genes.pdf")
for(i in rownames(named_nc_markers_activation)){
  boxcounts <- as.data.frame(t(named_nc_markers_activation))
  boxcounts <- as.data.frame(cbind(boxcounts[,i],coldata$condition))
  boxcounts$V2 <- coldata$condition
  boxplot <- ggplot(boxcounts, aes(x=V2, y=V1)) +
    geom_boxplot(color = c("red", "black")) +
...
pdf("barchart sigDE wnt inhibitor genes.pdf")
par(mfrow=c(2,2))
for(i in rownames(named_nc_markers_inhibition)){
  barcounts <- as.data.frame(t(named_nc_markers_inhibition))
...
pdf("barchart sigDE wnt activator genes.pdf")
par(mfrow=c(2,2))
for(i in rownames(named_nc_markers_activation)){
  barcounts <- as.data.frame(t(named_nc_markers_activation))
  barcounts <- barcounts[,i]
  barplot(barcounts, names.arg = "age group", main = i, 
          col = c(rep("light green",12), rep("light grey", 9)), ylab = "FPKM")
  legend(1,1, legend = c("young", "old"), pch = 15, col = c("light green", "light grey"))
...
colnames(named_nc_markers_activation) <- tp_age
colnames(named_nc_markers_inhibition) <- tp_age
test1_act <- named_nc_markers_activation
test1_inh <- named_nc_markers_inhibition

# calculate the mean of the columns with the same column name
test2 <- as.data.frame(
  sapply(unique(names(test1_act)), 
         function(col) rowMeans(test1_act[names(test1_act) == col]) # calculate row means
  )
)
